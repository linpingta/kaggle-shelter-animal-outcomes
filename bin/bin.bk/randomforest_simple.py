	...
	try:
		...
		if self.do_train:
			...
			for animal in animals:
				...
				clf_dict = {}
				print clf_name_dict.keys()
				for clf_name, clf in clf_name_dict.iteritems():
					logger.info('animal[%s] clf_name[%s] train model' % (animal, clf_name))
					print animal, 'train'
					if animal == 'All':
					...
					if True:
						print 'train model'
						logger.info('animal[%s] train model' % animal)
						# select model
						...
						print 'test1'
						logger.info('animal[%s] train process1' % animal)

						clf_dict[clf_name] = {'clf': clf,
						    'vectorizer_x': vectorizer_x,
						    'le_y': le_y
						}

						joblib.dump(clf, '.'.join([self.model_filename,animal, clf_name]))
						joblib.dump(vectorizer_x, '.'.join([self.vc_filename,animal, clf_name]))
						joblib.dump(le_y, '.'.join([self.le_filename,animal, clf_name]))

						print 'test2'
						logger.info('animal[%s] train process2' % animal)
						if self.do_validate:
							...
							logger.info('filename[%s] animal %s accrucy mean %0.2f +/- %0.2f' % (filename, animal, scores.mean(), scores.std()*2))

					animal_clf_dict[animal] = clf_dict

		logger.info('filename[%s] finish animal_clf_dict' % filename)

		clf_animal_dict = {}
		for animal, clf_dict in animal_clf_dict.iteritems():
			for clf_name, clf_info in clf_dict.iteritems():
				if clf_name not in clf_animal_dict:
					animal_dict = { animal: clf_info }
					clf_animal_dict[clf_name] = animal_dict
				else:
					clf_animal_dict[clf_name][animal] = clf_info
		logger.info('filename[%s] clf_animal_dict[%s]' % (filename, str(clf_animal_dict)))
		...
		if self.do_test:
			...
			for clf_name, animal_dict in clf_animal_dict.iteritems():
				logger.info('filename[%s] clf_name[%s] do test' % (filename, clf_name))
				...
				#s = encode_test_y * clf_weight_dict[clf_name]
				#y_submission = y_submission + encode_test_y * clf_weight_dict[clf_name]
				#y_submission = y_submission + encode_test_y * 1.0 / len(clf_name_dict.values())

				test_x = self._transfer_x(cleaned_test_data, vectorizer_x, '', total_info, logger)	
				print 'test_x', test_x.shape
				logger.info('test_x')
				clf_test_y = clf.predict_proba(test_x)
				y_submission = y_submission + clf_test_y * 1.0 / len(clf_name_dict.values())

			logger.info('filename[%s] merge submissions' % (filename))
			total_encode_test_y = normalize(y_submission, axis=1, norm='l1')
			output_y_list = self._output_y(total_encode_test_y, position_match_dict, logger)
...
class TsMultiModelClassifier(SimpleModel):
    def __init__(self, conf):
	super(TsMultiModelClassifier, self).__init__(conf)
	...
	if animal == 'Cat':
		rf_classifier = RandomForestClassifier(n_estimators=self.cat_sub_tree_num, max_depth=self.cat_max_depth_num)
	elif animal == 'Dog':
		rf_classifier = RandomForestClassifier(n_estimators=self.dog_sub_tree_num, max_depth=self.dog_max_depth_num)
	else:
		rf_classifier = RandomForestClassifier(n_estimators=self.sub_tree_num, max_depth=self.max_depth_num)
	xgb_classifier = XGBClassifier(learning_rate=0.04,
		n_estimators=200,
		#silent=False,
		max_depth=11,
		min_child_weight=3,
	...
	clf_name_dict =  { 
		...
		'xgb_classifier_2': xgb_classifier_2,
	}
